...
search_users_cmd = 'ldapsearch -LLL -h {server} -s sub -D "{user_dn}" -w {password} -x -b {base_dn} dn'
...
def main(server, write_password, password):
    ...
    search_cmd = search_users_cmd.format(server=server, user_dn=no_limits_user_dn,
                                         password=password, base_dn=base_dn)
    out = subprocess.Popen(search_cmd, stdout=subprocess.PIPE, shell=True).stdout.read()
    ...
    cmd = modify_cmd.format(server=server, user_dn=write_access_user_dn, password=write_password)
...
if __name__ == "__main__":
    ...
    password = raw_input("Enter password for user '{}': ".format(no_limits_user_dn))
    write_password = raw_input("Enter password for user '{}': ".format(write_access_user_dn))
