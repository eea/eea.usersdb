...
def main(server, user_dn, password):
    base_dn = "ou=Organisations,o=EIONET,l=Europe"
    search_cmd = search_orgs_cmd.format(server=server, user_dn=user_dn, password=password, base_dn=base_dn)
    ...
    cmd = modify_cmd.format(server=server, user_dn=user_dn, password=password)
...
if __name__ == "__main__":
    ...
    password = raw_input("Enter password for LDAP server: ")

    main(server, user_dn, password)
